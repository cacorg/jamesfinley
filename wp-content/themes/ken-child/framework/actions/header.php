...
add_action( 'fullscreen_navigation', 'mk_fullscreen_navigation' );
...
add_action( 'fullscreen_style_burger_icon', 'mk_fullscreen_style_burger_icon' );
add_action( 'fullscreen_container', 'mk_fullscreen_container' );
...
if ( !function_exists( 'mk_header_logo' ) ) {
	function mk_header_logo() {
		...
		$fullscreen_custom_logo = '';
		if(isset($mk_settings['header-structure']) && $mk_settings['header-structure'] == 'fullscreen'){
			$fullscreen_custom_logo = isset($mk_settings['fullheader-logo']['url']) ? $mk_settings['fullheader-logo']['url'] : '';
		}
		...
		$fullscreen_menu_css = (!empty($fullscreen_custom_logo)) ? 'fullscreen-custom-logo-exists' : '';

		$output = '<li class="mk-header-logo '.$mobile_logo_csss.' '. $fullscreen_menu_css .'">';
		...
		if ( !empty( $fullscreen_custom_logo ) ) {
			$output .= '<img alt="'.get_bloginfo( 'name' ).'" class="mk-fullscreen-logo" src="'.$fullscreen_custom_logo.'"  />';
		}
...
if ( !function_exists( 'mk_header_search' ) ) {
	function mk_header_search() {
		...
		if($mk_settings['header-search']){
			echo '
			' . (!empty( $mk_settings["header-social-facebook"]) ? '<li class="mk-header-facebook '.$header_location.'"><a href="' . $mk_settings["header-social-facebook"] . '" target="_blank"><i class="mk-icon-facebook"></i></a></li>' : '') . '
			<li class="mk-header-search '.$header_location.'">
...
?>
	<form method="get" class="header-searchform-input" action="<?php echo home_url(); ?>">
        <input class="search-ajax-input" type="text" value="" name="s" id="mk-ajax-search-input" placeholder="Enter search here" />
...
if ( !function_exists( 'mk_main_navigation' ) ) {
	function mk_main_navigation($menu_location) {
		...
		if($header_style == 'vertical' || $header_style == 'fullscreen') return false;
...
/*
* Create Full Screen Main Navigation
******/
if ( !function_exists( 'mk_fullscreen_navigation' ) ) {
	function mk_fullscreen_navigation($menu_location) {

		global $mk_settings;
		$header_style = $mk_settings['header-structure'];

		if($header_style != 'fullscreen') return false;
		...
		ob_start();
		do_action( 'fullscreen_style_burger_icon' );
		$output .= ob_get_contents();
		ob_end_clean();
...
/*
* Header Structure Full Screen burger icon
******/
if ( !function_exists( 'mk_fullscreen_style_burger_icon' ) ) {
	function mk_fullscreen_style_burger_icon() {

		global $mk_settings; 

		if($mk_settings['header-structure'] != 'fullscreen') return false;

		echo '<div class="mk-fullscreen-header-burger">
				<div class="mk-burger-icon">
	              <div class="burger-icon-1"></div>
	              <div class="burger-icon-2"></div>
	              <div class="burger-icon-3"></div>
...
* Header Structure Full Screen burger icon
******/
if ( !function_exists( 'mk_fullscreen_container' ) ) {
	function mk_fullscreen_container($menu_location) {

		global $mk_settings; 

		if($mk_settings['header-structure'] != 'fullscreen') return false;
		$fullscreen_menu_align = isset($mk_settings['fullheader-align']) ? $mk_settings['fullheader-align'] : 'left';
?>
<div class="mk-fullscreen-wrapper">
<div class="mk-fullscreen-overlay"></div>
<div class="mk-fullscreen-content <?php echo $fullscreen_menu_align; ?>-align">
...
?>	
	...
	<?php 
		echo wp_nav_menu( array(
				...
				'container' => 'div',
				'container_id' => false,
				'container_class' => 'fullscreen-menu-holder',
				'menu_class' => 'mk-fullscreen-menu ',
				'depth' => '1'
			));
	?>
