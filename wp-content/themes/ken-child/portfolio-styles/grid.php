...
function mk_portfolio_grid_loop( &$r, $atts, $current ) {
	...
	$manual = preg_replace('/manually[|][|]/', '', get_post_meta( get_the_ID(), '_portfolio_permalink', true ));
	$excerpt = ''; //substr(get_the_excerpt(get_the_ID()), 0, 100) . ' ...';
	if(empty($post_type)) {
		$post_type = 'image';
	}
	...
	if($hover_style == 'parallax'){
		...
		if($plus_icon == 'true') {
			$output .='<a href="'.(!empty($manual) ? $manual : $image_src_array[ 0 ]).'" title="'.get_the_title().'" ' . (!empty($manual) ? '' : 'data-fancybox-group="portfolio-loop"') . ' class="' . (!empty($manual) ? '' : 'mk-lightbox ') . 'portfolio-plus-icon">More Info</a>';
		}
		if($permalink_icon == 'true') {
			$output .='<a class="project-load portfolio-permalink buy-button" '.(!empty($manual) ? '' : 'data-post-id="'.get_the_ID().'"') . ' href="'.(!empty($manual) ? $manual : $permalink) .'">Buy!</a>';
		}
		$output .='</div>';
	}
	...
	$output .='<div class="hover-overlay"></div>';
		...
		if ($hover_style == 'classic'){
			if($plus_icon == 'true') {
				if(!empty($manual)) {
					$output .='<a href="' . (is_front_page() ? '/learn/books/' : '') . '#'.$post->ID.'" title="'.get_the_title().'" class="portfolio-plus-icon">More Info</a>';
				} else {
					$output .='<a href="' . (is_front_page() ? '/learn/books/' : '') . '#'.$post->ID.'" title="'.get_the_title().'" class="portfolio-plus-icon">More Info</a>';
				}
			}
			if($ajax == 'true') {
				$output .='<div class="the-title"><span>'.get_the_title().'</span></div><div class="the-excerpt">' . $excerpt . '</div><div class="clearboth"></div>';
			} else {
				if($permalink_icon == 'true') {
					if(!empty($manual)) {
						$output .='<div class="the-title"><a href="'.$manual.'">'.get_the_title().'</a></div><div class="the-excerpt">' . $excerpt . '</div><div class="clearboth"></div>';
					} else {
						$output .='<div class="the-title"><a href="'.$permalink.'">'.get_the_title().'</a></div><div class="the-excerpt">' . $excerpt . '</div><div class="clearboth"></div>';
					}
				} else {
					$output .='<div class="the-title">'.get_the_title().'</div><div class="the-excerpt">' . $excerpt . '</div><div class="clearboth"></div>';
				}
			}
			...
			if($permalink_icon == 'true') {
				if(!empty($manual)) {
					$output .='<a class="portfolio-permalink buy-button" target="_blank" href="'.$manual.'">Buy</a>';
				} else {
					$output .='<a class="project-load portfolio-permalink buy-button" data-post-id="'.get_the_ID().'" href="'. $permalink .'">Buy</a>';
				}
			...
			if($ajax == 'true') {
				...
				if ($hover_style == 'stroke') {
					if($permalink_icon == 'true') {
						if($manual) {
							$output .='<a href="'.$manual.'"><div class="the-title">'.get_the_title().'</div><div class="the-excerpt">' . $excerpt . '</div><div class="clearboth"></div></a>';
						} else {
							$output .='<a href="'.$permalink.'"><div class="the-title">'.get_the_title().'</div><div class="the-excerpt">' . $excerpt . '</div><div class="clearboth"></div></a>';
						}
					} else {
						$output .='<div class="the-title">'.get_the_title().'</div><div class="the-excerpt">' . $excerpt . '</div><div class="clearboth"></div>';
					}
			...
			if($permalink_icon == 'true') {
				if(!empty($manual)) {
					$output .='<a class="project-load buy-button" target="_blank" href="'.$manual.'">Buy</a>';
				} else {
					$output .='<a class="project-load portfolio-permalink buy-button" data-post-id="'.get_the_ID().'" href="'.$permalink.'">Buy</a>';
				}
			}
			if($plus_icon == 'true') {
				$output .='<a href="'.get_permalink().'" title="'.get_the_title().'" class="mk-lightbox portfolio-plus-icon">More Info</a>';
			}
			$output .= '</div>';
		}
