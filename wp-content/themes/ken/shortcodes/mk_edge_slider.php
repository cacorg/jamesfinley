...
$output .= '<div id="mk-edge-slider-'.$id.'" class="mk-edge-slider '.$visibility.' mk-swiper-container first-el-' . $first_el . '" style="height:' . $height . 'px;" data-first="' . $first_el . '" data-loop="'.$edge_slider_loop.'" data-height="' . $height . '" data-fullHeight="' . $full_height . '" data-pause="' . $slideshow_speed . '" data-speed="' . $animation_speed . '"  data-pagination="'.$pagination_class.'" data-animation="'.$animation_effect.'">';
...
while ($loop->have_posts()):
    ...
    $title_size_number = get_post_meta($post->ID, '_title_size', true);
    if( $title_size_number > $max_title_size){
        $max_title_size = $title_size_number;
    }
    if($content_width < $min_content_width ){
        $min_content_width = $content_width;
    }
...
if($max_title_size > 80 && $min_content_width < 70){

$app_styles = '
            @media handheld, only screen and (max-width: 1024px) {
                #mk-edge-slider-'.$id.' .mk-edge-title {
