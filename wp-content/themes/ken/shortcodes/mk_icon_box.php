...
$output .= '<h4 class="icon-box-title">' . $title . '</h4>';

$output .= '<div class="icon-box-desc">' . wpb_js_remove_wpautop($content, true) . '</div>';
if (!empty($read_more_url) && !empty($read_more_txt)) {
	$button_align = ($style == 'style2') ? 'left' : 'center';
	$output .= do_shortcode('[mk_button style="outline" outline_skin="' . $btn_skin . '" outline_hover_skin="' . $btn_hover_skin . '" outline_border_width="2" size="small" align="' . $button_align . '" url="' . $read_more_url . '" target="'.$url_target.'"]' . $read_more_txt . '[/mk_button]');
}
...
Mk_Static_Files::addCSS("
    #icon-box-{$id} .icon-box-title {
        color:{$title_color};
        font-size:{$title_text_size}px
    }
    #icon-box-{$id} .icon-box-desc, 
    #icon-box-{$id} .icon-box-desc p{ 
        font-size:{$p_text_size}px;
        color:{$txt_color}
    }
"
, $id);
