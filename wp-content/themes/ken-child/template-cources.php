							...
							<?php $manual = preg_replace('/manually[|][|]/', '', get_post_meta( get_the_ID(), '_portfolio_permalink', true )); ?>
								...
								<?php if($nCount++%2 == 0) { ?>
									...
									<div class="vc_col-sm-7">
										...
										<div class="excerpt">
											<?php the_excerpt(); ?>
											<div class="mk-button-align center"><a href="<?php echo (!empty($manual) ? $manual : get_post_permalink($post->ID)); ?>" <?php echo (!empty($manual) ? 'target="_blank"' : ''); ?> class="mk-button btn-2  flat-button small pointed   "><span>GET STARTED</span></a></div>
										</div>
								</div>
								<?php } else { ?>
									<div class="vc_col-sm-7">
